...
## Low-level audit policy

Batch 3 decodes emitted bytes before comparing them with source-derived
invariants.  EXTH fields without an external contract are inventory-only: the
suite checks framing and boundedness but does not impose an unproven
KindleGen-byte value.  The regression mapping in
`tests/audit_support/regression_traceability.md` records the requirement and
product-contract basis for each retained compatibility assertion.
